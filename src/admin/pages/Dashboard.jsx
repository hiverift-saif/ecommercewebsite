...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const response = await fetch(`${BASE.BASE_URL}/dashboard`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            // Agar token chahiye to yahan add kar dena
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-bold">${todaysSales.toLocaleString()}</p>
              </div>
                ...
                {recentOrders.length === 0 ? (
                  ...
                  recentOrders.map((order, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-4 px-6">{order.orderId || `#ORD-${order._id?.slice(-6)}`}</td>
                      <td className="py-4 px-6">{order.customerName || 'N/A'}</td>
                      <td className="py-4 px-6">{order.productName || 'Multiple Items'}</td>
                      <td className="py-4 px-6">${order.amount?.toFixed(2) || '0.00'}</td>
