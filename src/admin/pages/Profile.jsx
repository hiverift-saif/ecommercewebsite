...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const res = await fetch(`${BASE.BASE_URL}/profile`, {
          method: "GET",
  ...
  const validate = () => {
    ...
    else if (!/^\d{10}$/.test(form.phone)) newErr.phone = "Phone must be 10 digits.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE.BASE_URL}/profile/update`, {
        method: "PUT",
        headers: {
  ...
  const handleImage = async (e) => {
    ...
    try {
      const res = await fetch(`${BASE.BASE_URL}/profile/upload-image`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
  ...
  const updatePassword = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE.BASE_URL}/profile/change-password`, {
  ...
  return (
    <main className="p-6 space-y-6">
      ...
      <div className="bg-white border p-6 rounded-xl space-y-4">
        ...
        <button onClick={handleSubmit} className="bg-black text-white px-4 py-2 rounded-md flex items-center gap-2">
      ...
      <div className="bg-white border p-6 rounded-xl space-y-4">
        ...
        <div className="flex items-center gap-6">
          <div className="w-28 h-28 border rounded-md overflow-hidden bg-gray-100 flex items-center justify-center">
            {preview ? <img src={preview} className="w-full h-full object-cover" alt="" /> : <p className="text-gray-400">No image</p>}
          </div>
          ...
          <div>
            ...
            {preview && (
              <button onClick={removeImage} className="text-red-600 mt-2 flex items-center gap-2">
      ...
      <div className="bg-white border p-6 rounded-xl space-y-4">
        ...
        <div>
          ...
          {passErrors.oldPass && <p className="text-red-500 text-xs">{passErrors.oldPass}</p>}
        </div>
        ...
        <div>
          ...
          {passErrors.newPass && <p className="text-red-500 text-xs">{passErrors.newPass}</p>}
        </div>
        ...
        <div>
          ...
          {passErrors.confirmPass && <p className="text-red-500 text-xs">{passErrors.confirmPass}</p>}
        </div>

        <button onClick={updatePassword} className="bg-black text-white px-4 py-2 rounded-md flex items-center gap-2">
